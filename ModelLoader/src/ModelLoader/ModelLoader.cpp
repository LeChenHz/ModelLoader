...
void ModelLoader::initVulkan() {
	...
	initBuffers();
	createTextureImage(0); //纹理对象
	createTextureImageView(0);//纹理图像视图
...
void ModelLoader::mainLoop() {
	...
	while (!glfwWindowShouldClose(window))
	{
...

		{
			...
			{
				OPENFILENAME modelOpen;
				char modelFile[MAX_PATH];
				loadFile(modelOpen, modelFile);
				if (GetOpenFileName(&modelOpen))
				{
					...
					if (IsModelLoaded)
					{
						if (!First_Loaded)
						{
							CURRENT_MODEL_VERTEXBUFFER_INDEX = (CURRENT_MODEL_VERTEXBUFFER_INDEX + 1) % MODEL_VERTEXBUFFER_COUNT;
						}
						createVertexBuffer(CURRENT_MODEL_VERTEXBUFFER_INDEX);
						createIndexBuffer(CURRENT_MODEL_VERTEXBUFFER_INDEX);
						First_Loaded = false;
						Clean_Last_Buffer = true;
					}
				}
			}
			if (ImGui::Button("Select Texture"))                           
			{
				OPENFILENAME TextureOpen;
				char TextureFile[MAX_PATH];
				loadFile(TextureOpen, TextureFile);
				if (GetOpenFileName(&TextureOpen))
				{
					TEXTURE_PATH = TextureFile;
					if (IsTextureLoaded)
					{
						CURRENT_TEXTURE_INDEX = (CURRENT_TEXTURE_INDEX + 1) % 2;
						createTextureImage(CURRENT_TEXTURE_INDEX);
						createTextureImageView(CURRENT_TEXTURE_INDEX);
						UpdateDescriptorSets();
						Clean_Last_Texture = true;
					}
				}
			}
		...
		{
			ImGui::Begin("Rotate");
			ImGui::gizmo3D("##gizmo1", PanDolly, qRot);
			ImGui::End();
		}
		...
		{
			vkDestroyBuffer(device, indexBuffer[(CURRENT_MODEL_VERTEXBUFFER_INDEX + 1) % MODEL_VERTEXBUFFER_COUNT], nullptr);
			vkFreeMemory(device, indexBufferMemory[(CURRENT_MODEL_VERTEXBUFFER_INDEX + 1) % MODEL_VERTEXBUFFER_COUNT], nullptr);
			
			vkDestroyBuffer(device, vertexBuffer[(CURRENT_MODEL_VERTEXBUFFER_INDEX + 1) % MODEL_VERTEXBUFFER_COUNT], nullptr);
			vkFreeMemory(device, vertexBufferMemory[(CURRENT_MODEL_VERTEXBUFFER_INDEX + 1) % MODEL_VERTEXBUFFER_COUNT], nullptr);
			Clean_Last_Buffer = false;
		}

		if (Clean_Last_Texture)
		{
			vkDestroyImageView(device, textureImageView[(CURRENT_TEXTURE_INDEX + 1) % 2], nullptr);
			vkDestroyImage(device, textureImage[(CURRENT_TEXTURE_INDEX + 1) % 2], nullptr);
			vkFreeMemory(device, textureImageMemory[(CURRENT_TEXTURE_INDEX + 1) % 2], nullptr);
			Clean_Last_Texture = false;
		}
	}
...
void ModelLoader::cleanup() {
	...
	vkDestroyImageView(device, textureImageView[CURRENT_TEXTURE_INDEX], nullptr);

	vkDestroyImage(device, textureImage[CURRENT_TEXTURE_INDEX], nullptr);
	vkFreeMemory(device, textureImageMemory[CURRENT_TEXTURE_INDEX], nullptr);

	vkDestroyDescriptorSetLayout(device, descriptorSetLayout, nullptr);

	vkDestroyBuffer(device, indexBuffer[CURRENT_MODEL_VERTEXBUFFER_INDEX], nullptr);
	vkFreeMemory(device, indexBufferMemory[CURRENT_MODEL_VERTEXBUFFER_INDEX], nullptr);

	vkDestroyBuffer(device, vertexBuffer[CURRENT_MODEL_VERTEXBUFFER_INDEX], nullptr);
	vkFreeMemory(device, vertexBufferMemory[CURRENT_MODEL_VERTEXBUFFER_INDEX], nullptr);
...
void ModelLoader::createImGuiDescriptorPool() {
	...
	if (vkCreateDescriptorPool(device, &pool_info, nullptr,
		&imgui_descriptorPool) != VK_SUCCESS) {
		throw std::runtime_error("failed to create imgui_descriptor pool!");
	}
}
...
void ModelLoader::UpdateDescriptorSets() {
	for (size_t i = 0; i < swapChainImages.size(); i++)
	{
		/* 指定缓冲对象、偏移、访问的数据范围，range若要访问整个缓冲可以设置VK_WHOLE_SIZE */
		VkDescriptorBufferInfo bufferInfo = {};
...
void ModelLoader::initBuffers()
{
	vertexBuffer.resize(MODEL_VERTEXBUFFER_COUNT);
	vertexBufferMemory.resize(MODEL_VERTEXBUFFER_COUNT);
	indexBuffer.resize(MODEL_VERTEXBUFFER_COUNT);
	indexBufferMemory.resize(MODEL_VERTEXBUFFER_COUNT);

	textureImage.resize(2);
	textureImageMemory.resize(2);
	textureImageView.resize(2);
}

void ModelLoader::createVertexBuffer(int index) {
	if (IsModelLoaded)
	{
		...
		/* 显卡读取快的真正缓冲 */
		createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, vertexBuffer[index], vertexBufferMemory[index]);
...
void ModelLoader::createIndexBuffer(int index) {
	if (IsModelLoaded)
	{
		...
		createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, indexBuffer[index], indexBufferMemory[index]);
...
void ModelLoader::imguiFrameRender(ImDrawData* draw_data, uint32_t imageindex, int model_index) {
	VkCommandBufferBeginInfo beginInfo{};
	beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
	beginInfo.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
	...
	VkBuffer vertexBuffers[] = { vertexBuffer[model_index] };
	VkDeviceSize offsets[] = { 0 };
	if (IsModelLoaded)
	{
		vkCmdBindVertexBuffers(commandBuffers[imageindex], 0, 1, vertexBuffers, offsets);

		vkCmdBindIndexBuffer(commandBuffers[imageindex], indexBuffer[model_index], 0, VK_INDEX_TYPE_UINT32);//最后的参数必须和索引的数组类型一致
...
void ModelLoader::createTextureImage(uint32_t imageIndex) {
	...
	createImage(texWidth, texHeight, mipLevels, VK_SAMPLE_COUNT_1_BIT, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL,
		...
		VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, textureImage[imageIndex], textureImageMemory[imageIndex]);
	/* 变换布局、复制缓冲区数据 */
	transitionImageLayout(textureImage[imageIndex], VK_FORMAT_R8G8B8A8_SRGB,
		VK_IMAGE_LAYOUT_UNDEFINED,
		VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipLevels);
	copyBufferToImage(stagingBuffer, textureImage[imageIndex],
	...
	generateMipmaps(textureImage[imageIndex], VK_FORMAT_R8G8B8A8_SRGB, texWidth, texHeight, mipLevels);
}

void ModelLoader::createTextureImageView(uint32_t imageIndex) {
	textureImageView[imageIndex] = createImageView(textureImage[imageIndex], VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels);
}
...
void ModelLoader::drawFrame() {
	...
	imguiFrameRender(imgui_draw_data, imageIndex, CURRENT_MODEL_VERTEXBUFFER_INDEX);
...
void ModelLoader::updateUniformBuffer(uint32_t currentImage) {
	...
	glm::mat4 model;
	ubo.model = vgmCast(modelMatrix, model);

	//ubo.model = glm::scale(model, glm::vec3(scale));
...
{
	glm::vec4 line0(vgmModel.m00, vgmModel.m01, vgmModel.m02, vgmModel.m03);
	glm::vec4 line1(vgmModel.m10, vgmModel.m11, vgmModel.m12, vgmModel.m13);
	glm::vec4 line2(vgmModel.m20, vgmModel.m21, vgmModel.m22, vgmModel.m23);
	glm::vec4 line3(vgmModel.m30, vgmModel.m31, vgmModel.m32, vgmModel.m33);

	model = glm::mat4(line0, line1, line2, line3);
...
void ModelLoader::loadModel() {
	...
	{
		float scale = findScale(attrib.vertices);
		for (const auto& index : shape.mesh.indices) {
			Vertex vertex = {};
			vertex.pos = {
				scale * attrib.vertices[3 * index.vertex_index + 0],//x
				scale * attrib.vertices[3 * index.vertex_index + 1],//y
				scale * attrib.vertices[3 * index.vertex_index + 2] //z
	...
	setVertexNums(attrib.vertices.size());
	setTriangles(indices.size()/3);
}

void ModelLoader::setVertexNums(int num)
{
	VertexNums = num;
}

void ModelLoader::setTriangles(int num)
{
	Triangles = num;
}

int ModelLoader::getVertexNums()const
{
	return VertexNums;
}

int ModelLoader::getTriangles()const
{
	return Triangles;
}

float ModelLoader::findScale(std::vector<float>& vec)
{
	float res = 1;
	float max = -1;
	for (int i = 0; i < vec.size(); i++)
	{
		max = std::abs(vec[i]) > max ? abs(vec[i]) : max;
	}
	while (max > 1)
	{
		max = max / 10;
...
void ModelLoader::loadFile(OPENFILENAME& file, char* filePath)
{
